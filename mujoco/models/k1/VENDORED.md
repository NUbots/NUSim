...
## Scene: `k1_scene_robocup_middle.xml`
...
"Dimensions", table "Exemplary field dimensions by field type"): field
14 x 9 m + 1 m border, goal area 1 x 4 m, penalty area 3 x 6 m, penalty mark
2 m from the goal line, center circle diameter 3 m, corner arcs radius 0.5 m,
lines 0.05 m, marks 0.10 m. The rules give ranges for the goal (width
2.4-2.6 m, height 1.5-1.9 m, depth 0.7-1.2 m); this one is 2.5 m between the
inner post edges with the crossbar centred at 1.8 m, and like the KidSize
goals it has no net. The Middle Division plays with a FIFA size 3 or 4 ball;
the scene keeps the KidSize ball the Yolo model was trained on.

The pitch is generated by `generate_field.py` (`python3
models/k1/generate_field.py middle`), which also reproduces the KidSize
scene's markings and goals exactly (`kid`), so new fields come out in the same
conventions: lines are box geoms centred on the nominal dimensions (the rules
measure to the outside of the lines, 2.5 cm further out), and circles and arcs
are 9 degree segments overlapping by 15%.

`config/supervisor.yaml`'s poses assume the KidSize field (its `penalty_pose`
at y = 3.5 is just outside the KidSize touchline but inside the M-Field), so
check them before enabling the supervisor on this scene.
