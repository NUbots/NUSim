...
Training is **MJX (JAX), GPU-parallel**, in the same MuJoCo physics used for deployment (no sim2sim gap):

```bash
./b train walk.py               # train a policy (see mujoco/training/README.md)
```

The trained policy exports to ONNX and is loaded by the C++ locomotion backend
(`config/locomotion.yaml: backend: policy`, built `-DK1_WITH_ONNX=ON`). Framework, `Policy` base class,
and the obs/action contract are under **[mujoco/training/](mujoco/training/)**.

## Layout

| Path | Description |
| --- | --- |
| `b`, `mujoco/b.py`, `mujoco/tools/` | NUbots-style `./b` command dispatcher (`run`, `build`, `configure`, `train`). |
...
| `mujoco/training/` | MJX training framework (`Policy` base class, `policies/`, `train.py`, ONNX export). |
